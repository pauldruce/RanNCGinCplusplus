...
/* get_chirality():
 ...
 Once we have constructed all the clifford generators, they are stored in
...
cx_mat Clifford::get_chirality()
{
   ...
   for (const auto &gam : generators)
      c *= gam;
   c.clean(datum::eps);
   return c;
}

cx_mat Clifford::generate_chirality(vector<cx_mat> &input_generators) const
{
   cx_mat c = eye<cx_mat>(matrix_size, matrix_size);
   c *= std::pow(std::complex<double>(0, 1), ssp1 / 2);
   for (const auto &gam : input_generators)
      c *= gam;
   c.clean(datum::eps);
   return c;
}
...
void Clifford::setup()
{
   ...
   if (n > 2)
   {
      ...
      for (int i = 0; i < d_p; i++)
      {
         ...
         // product-ing.
         for (const auto &gen : cliff20.generators)
         {
            holder_gens.emplace_back(kron(gen, eye<cx_mat>(module_dim, module_dim)));
         }
         for (const auto &gen : input_gens)
         {
            holder_gens.emplace_back(kron(cliff20.chirality, gen));
         }
         // After product-ing with a (2,0) the module_dim =
      ...
      for (int i = 0; i < d_q; i++)
      {
         // This gives me the matrix dimension of the second module we are product-ing.
         for (const auto &gen : cliff02.generators)
         {
            holder_gens.emplace_back(kron(gen, eye<cx_mat>(module_dim, module_dim)));
         }
         for (const auto &gen : input_gens)
         {
            ...
            holder_gens.emplace_back(kron(cliff02.chirality, gen));
         }
         // After product-ing with a (2,0) the module_dim = matrix_dimension is increased by a factor of 2
         module_dim *= 2;
         // Set the input_gens to be the just constructed generators, so we can then repeat the process if necessary.
         input_gens = holder_gens;
      }

      // Order the generators so the Hermitian elements are first, then the anti-Hermitian
      vector<cx_mat> herm;
